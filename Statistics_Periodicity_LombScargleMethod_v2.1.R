...
##
############################################################################################################################
## Version 2:  The new feature added on this code is to implement the comparison of std on the Power for MCS a
##             UWS
############################################################################################################################
...
file_mat_LS_ultr_4d = 'result_LombScarge_circ_4days_4.mat'
...
vector_values <- seq(2,n_mcs, by=3)
...
#######################################
#######################################
### Creating MCS -std  and UWS-std ####
#######################################
#######################################

# This part of the code creates a dataframe where it has 
# the value of std per subject (computer over the power for the periodicities)
#


# declaring the data frames that will contain the results

table_Circ_Std_Power <- data.frame(ID_ = NULL, Std_Power = NULL, Diagnosis = NULL)


for(i in width_mcs_power){ # for loop over the number of subject on MCS 
  power_i <- power_MCS[,i]
  std_power_i<- sd(power_i)
  table_Circ_Std_Power_temp <- data.frame(Std_Power = std_power_i,Diagnosis = "MCS")
  table_Circ_Std_Power <- rbind(table_Circ_Std_Power, table_Circ_Std_Power_temp)
}

for(j in width_uws_power){ # for loop over the number of subject on UWS 
  power_j <- power_UWS[,j]
  std_power_j<- sd(power_j)
  table_Circ_Std_Power_temp <- data.frame(Std_Power = std_power_j,Diagnosis = "UWS")
  table_Circ_Std_Power <- rbind(table_Circ_Std_Power, table_Circ_Std_Power_temp)
}

xx <-table_Circ_Std_Power$Diagnosis
yy<- table_Circ_Std_Power$Std_Power

function Actigraphy_Stats_v2_6_single_level_msa_se_(segment)
...
elseif(strcmp(segment, 'comparison_msa_se_whole_day'))
    ...
    plot_statistics_paper1(path_code, path_control_mtn_se, path_emcs_mtn_se, path_mcs_mtn_se, path_uws_mtn_se, MSA)
...
%     figure, plot(t_control_mtn, mean(MCS_mtn_aux))
    ...
    figure,
    plot(t_control_mtn, mean(MCS));
    legend(hh, 'MCS')
    title( 'Plot of MCS ','FontSize',14,'FontWeight','bold','Color','k')
    ylabel('Spectral Amplitude (SA)', 'FontSize',14,'FontWeight','bold','Color','k')
    xlabel('time (min)');
    ylim([0.0 1.2])
    ...
    errorbar (t_mcs_mtn, y4_CC,z4_CC, 'b'); grid on; 
    title( 'Plot of MSA on MCS subjects')
    ylabel('Spectral Amplitude (SA)', 'FontSize',14,'FontWeight','bold','Color','k')
    xlabel('time (min)');
    ylim([0.0 1.2])
 
    
    % Plot of all with the error bar or confidence interval with lines
    % according with suggestion of Audrey Wolff
    
    figure;
    y4_CC = mean (MCS);
    z4_CC = std (MCS)/sqrt (length(MCS));
    plot(t_control_mtn, y4_CC, 'b', t_control_mtn , y4_CC + z4_CC, 'r', t_control_mtn, y4_CC - z4_CC, 'r');  grid on; 
    title( 'Plot of MSA on MCS subjects')
    ylabel('Spectral Amplitude (SA)', 'FontSize',14,'FontWeight','bold','Color','k')
    xlabel('time (min)');
    ylim([0.0 1.2])
    ...
    [m_MCS_SE_awd, n_MCS_SE_awd] = size(EMCS_mtn_SE_aux);  % SE
    ...
    [m_MCS_SE_mtn, n_MCS_SE_mtn] = size(EMCS_mtn_SE_aux);  % SE
    ...
    for i=1:m_MCS_mtn
        ...
        % previously:
         ...
         mcs_mtn_selected_pks_values{i}.std_SE = std(MCS_awd_SE_aux(i,:));
         mcs_mtn_selected_pks_values{i}.mean_SE = mean(MCS_awd_SE_aux(i,:));
    ...
    SE_at_indx_m_MCS_awd = zeros(m_MCS_SE_mtn,3);
    ...
    figure,
    hh = plot(t_control_mtn, mean(UWS));
    legend(hh, 'MCS')
    title( 'Plot of MCS ','FontSize',14,'FontWeight','bold','Color','k')
    ylabel('Spectral Amplitude (SA)', 'FontSize',14,'FontWeight','bold','Color','k')
    xlabel('time (min)');
    ...
    figure;
    y4_CC = mean (UWS);
    z4_CC = std (UWS)/sqrt (length(UWS));
    errorbar (t_mcs_mtn, y4_CC,z4_CC, 'b'); grid on; 
    title( 'Plot of MSA on UWS subjects', 'FontSize',20,'FontWeight','bold','Color','k')
    ylabel('Spectral Amplitude (SA)', 'FontSize',30,'FontWeight','bold','Color','k')
    xlabel('time (min)', 'FontSize',30,'FontWeight','bold','Color','k')
    %xlabel('time (min)');
    ylim([0.0 1.2])
    savefig(h, 'MCS_UWS.fig');
    ...
    z4_CC = std (UWS)/sqrt (length(UWS));
    plot(t_control_mtn, y4_CC, 'b', t_control_mtn , y4_CC + z4_CC, 'r', t_control_mtn, y4_CC - z4_CC, 'r');  grid on; 
    title( 'Plot of MSA on UWS subjects')
    ylabel('Spectral Amplitude (SA)', 'FontSize',14,'FontWeight','bold','Color','k')
    xlabel('time (min)');
    ylim([0.0 1.2])
    ...
    for i=1:m_UWS_awd
        ...
        number_of_pks_UWS_awd = [number_of_pks_UWS_awd; length(indx_pks_UWS_awd)];
         ...
         uws_awd_selected_pks_values{i}.std_SE = std(MCS_awd_SE_aux(i,:));
         uws_awd_selected_pks_values{i}.mean_SE = mean(MCS_awd_SE_aux(i,:));
...
%     plot( t_control_mtn, UWS_awd_aux(i,:))
    ...
    SE_at_indx_m_UWS_awd = zeros(m_UWS_SE_awd,3);
...
%     plot( t_control_mtn, UWS_mtn_aux(i,:))
   ...
   SE_at_indx_m_UWS_mtn = zeros(m_UWS_SE_mtn,3);

function [M,Amp,phi, p_3a] = cosinor(t,y,w,alpha,show_graphics)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   show_graphics - logical value: true if you want to show the graphics
%                   and false if you don't want to show the graphics
%
% Define Variables:
...
NE = [  n        sum(x)       sum(z)     nansum(y); %% mofified by Aldo Camargo b(ULG - Belgium) because it was not working with NAN numbers, it was before sum(y)
      sum(x)   sum(x.^2)    sum(x.*z)    nansum(x.*y);
      sum(z)   sum(x.*z)    sum(z.^2)    nansum(z.*y);];
...
RSS = nansum((y - (M + beta.*x + gamma.*z)).^2);  % Modified by Aldo Camargo (ULG) it was sum
...
[CI_Amp_min, CI_Amp_max, CI_phi_min, CI_phi_max] = CIcalc(X,T,Z,beta,gamma,n,sigma,Amp,phi,alpha,show_graphics ); %#ok<NASGU,NASGU>
